...
CONFIG_SQL = [
  ...
  "tags_users.sql",
  "tickets.sql",
  "guild_configuration.sql",
  "assigned_tickets.sql",
  "sessions.sql"
]
...
#  INSERTS-AUTH: ...
  ...
  def insert_sessions(self, session_id, token, refresh_token, expires_in, user_id):
    if not self.db_mysql_connector.is_connected():
      self.reconnect()
    expires=datetime.now() + timedelta(seconds=expires_in)
    expires_str = expires.strftime("%Y-%m-%d %H:%M:%S")
    query = f"INSERT INTO sessions (session_id, token, refresh_token, token_expires_at, user_id) VALUES (%s, %s, %s, %s, %s)"
    self.cursor.execute(query, (session_id, token, refresh_token, expires_str, user_id))
    self.db_mysql_connector.commit()
    return True

  def get_sessions(self, session_id):
    print(session_id)
    self.cursor.execute("SELECT token, refresh_token, token_expires_at, user_id FROM sessions WHERE session_id = %s", (session_id,))
    result = self.cursor.fetchone()
    if result:
      token, refresh_token, token_expires_at, user_id = result
      return {
        "token": token,
...
# -----------------------------------------------------------
  ...
  def execute_sql_files(self):
    ...
    sql_files_path = os.path.join(parent_directory, 'database')
    files_path = os.path.join(sql_files_path, 'entities')
    print(files_path)
    for files in CONFIG_SQL:
      file_path = os.path.join(files_path, files)
...
@router.get(path='/database/create_tables')
async def create_tables():
  dbMaria.execute_sql_files()
...
# Logs
ticket_channel_request = 1212901550301646848
channel_logs_tickets_service = 1212890405574541342
# Category
services_category = 1213091206628778074
...
class modal_window_ticket_system_report(discord.ui.Modal, title="📌🞄 заполните пункты для: репортов"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await channel.send(embed=embed_ticket_player)
        embed_message_control_tickets = discord.Embed(title=f"Тикет ID:``{channel.id}``", description=f"{interaction.user.mention} создал тикет - по услугам")
        embed_message_control_tickets.add_field(name=f"Где находится : ", value=f"Канал: {channel.mention}")
        
        control_message = interaction.guild.get_channel(config.ticket_system_report_channel_request)
        
        if control_message:
            await control_message.send(embed=embed_message_control_tickets, view=buttons_on_control_ticket_by_moderator())
        else:
            print(f"Channel with ID {config.ticket_system_report_channel_request} not found.")
...
class buttons_on_control_ticket_by_moderator(View):
    ...
    @discord.ui.button(label="Информация", style=discord.ButtonStyle.gray, custom_id="ticket_button_info")
    @app_commands.checks.has_permissions(administrator=True)
    async def info_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        pass
    @discord.ui.button(label="Закрыть (Адмн)", style=discord.ButtonStyle.red, custom_id="ticket_button_reject")
    @app_commands.checks.has_permissions(administrator=True)
    async def reject_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        pass
    @discord.ui.button(label="Принять", style=discord.ButtonStyle.green, custom_id="ticket_button_consideration")
    @app_commands.checks.has_permissions(administrator=True)
